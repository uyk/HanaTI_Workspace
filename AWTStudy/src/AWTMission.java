...
public class AWTMission extends Panel implements WindowListener{
	...
	public void setContents() {
		...
		addToGridBag(subjectTextField,  1, 2, 3, 1, 1, 0);
		
		addToGridBag(contentsTextArea,  0, 3, 4, 1, 1, 1);
		
		bottomPanel.add(sendButton);
		bottomPanel.add(cancelButton);
		
		addToGridBag(bottomPanel,  0, 4, 4, 1, 0, 0);
		addToGridBag(new Button("aa"),  0, 5, 1, 2, 0, 0);
...
/* 마진 설정	
	private void addToGridBag(Component component, int gridx, int gridy, int gridwidth, int gridheight, double weightx, double weighty, Insets insets) {
		...
		gridBagConstraints.fill = GridBagConstraints.BOTH;	// 호리즌탈(가로채움), 버티컬(새로채움), BOTH(전체채움), NONE(디폴트)
		gridBagConstraints.insets = insets;			//마진
		gridBagLayout.setConstraints(component, gridBagConstraints);
...
*/
	...
	public static void main(String[] args) {
		...
		AWTMission panel = new AWTMission();
